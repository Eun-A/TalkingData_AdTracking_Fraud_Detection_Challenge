...
train_data <- fread("train.csv", sep = ",", header = TRUE)
# test_data <- fread("test_data.csv", sep = ",", header = TRUE)
# test_sup <- fread("test_supplement.csv", sep = ",", header = TRUE)
# # sample_submission <- fread("sample_submission.csv", sep = ",", header = TRUE)
...
## 1-2 데이터 분할
# : Train데이터 날짜별로 3개로 분할 후 저장 

train_data <- as.data.table(train_data)
train_data <- train_data[, -7] # attributed_time 변수 삭제 
...
# 파일로 저장 
# write.csv(day7_train, "day7_train.csv")
# write.csv(day7_valid, "day7_valid.csv")
# write.csv(day8_train, "day8_train.csv")
# write.csv(day8_valid, "day8_valid.csv")
# write.csv(day9_train, "day9_train.csv")
# write.csv(day9_valid, "day9_valid.csv")
...
###############################################################################################
전체 train데이터를 날짜별로 나눠서 각각 lightgbm으로 모델링 한 후 결과값을 stacking 하는 방법을
시도해 보았으나 단일 모델을 사용한 것과 별 차이가 없어 day9 데이터만 사용하여 학습, 예측하였다.
###############################################################################################

## 3-1 lightgbm으로 모델링하기
